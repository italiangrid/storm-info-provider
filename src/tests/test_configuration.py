...
from tests.utils import (
    get_default_test_configuration,
...
class TestConfiguration(unittest.TestCase):
    def _check_is_test_configuration(self, configuration, filepath):
        logging.debug(
            "Configuration check: verify %s contains values from file %s",
            configuration,
            filepath,
        )
        ...
        self.assertEqual(
            configuration.get_webdav_endpoints(),
            ["https://xfer-test.cr.cnaf.infn.it:8443"],
        )
        ...
        self.assertEqual(
            configuration.get_storage_area_list(),
            [
                "infotest",
                "ops",
                "testgrouptape",
    ...
    def test_load_default_configuration(self):
        ...
        self._check_is_test_configuration(
            configuration, get_default_test_configuration_filepath()
        )
