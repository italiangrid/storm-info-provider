...
class TestGlue2(unittest.TestCase):
    def _check_generated_storage_service(self, node, configuration):
        ...
        self.assertEqual(
            decode_list(node["entries"]["GLUE2ServiceQualityLevel"]),
            [configuration.get_quality_level()],
        )
        self.assertEqual(
            decode_list(node["entries"]["GLUE2ServiceAdminDomainForeignKey"]),
            [configuration.get_sitename()],
        )
        self.assertEqual(
            decode_list(node["entries"]["objectClass"]),
            ["GLUE2Service", "GLUE2StorageService"],
        )
        self.assertEqual(decode_list(node["entries"]["GLUE2ServiceType"]), ["storm"])
        self.assertEqual(
            decode_list(node["entries"]["GLUE2ServiceCapability"]),
            ["data.management.storage"],
        )
        self.assertEqual(
            decode_list(node["entries"]["GLUE2EntityOtherInfo"]),
            ["ProfileName=EGI", "ProfileVersion=1.0"],
        )
    ...
    def _check_generated_storage_service_capacity(self, node, configuration, spaceinfo):
        ...
        self.assertIsNotNone(node["entries"]["GLUE2EntityCreationTime"])
        self.assertEqual(
            decode_list(node["entries"]["GLUE2StorageServiceCapacityID"]), [expected_id]
        )
        self.assertEqual(
            decode_list(node["entries"]["objectClass"]), ["GLUE2StorageServiceCapacity"]
        )
        self.assertEqual(
            decode_list(
                node["entries"]["GLUE2StorageServiceCapacityStorageServiceForeignKey"]
            ),
            [service_id],
        )
        self.assertEqual(
            decode_list(node["entries"]["GLUE2StorageServiceCapacityType"]), ["online"]
        )
        self.assertEqual(
            decode_list(node["entries"]["GLUE2StorageServiceCapacityTotalSize"]),
            [str(as_gigabytes(spaceinfo.get_summary().get_total()))],
        )
        self.assertEqual(
            decode_list(node["entries"]["GLUE2StorageServiceCapacityFreeSize"]),
            [str(as_gigabytes(spaceinfo.get_summary().get_free()))],
        )
        self.assertEqual(
            decode_list(node["entries"]["GLUE2StorageServiceCapacityUsedSize"]),
            [str(as_gigabytes(spaceinfo.get_summary().get_used()))],
        )
        self.assertEqual(
            decode_list(node["entries"]["GLUE2StorageServiceCapacityReservedSize"]),
            [str(as_gigabytes(spaceinfo.get_summary().get_reserved()))],
        )
    ...
    def test_static_ldif(self):
        ...
        self._check_generated_storage_service_capacity(
            nodes[1].get_info(), configuration, spaceinfo
        )
